...
export default function TaxonomyPage() {
  ...
  const handleSidebarStepClick = (index: number) => {
    const stepRoutes = [
      "/attribution-lite/new?step=campaign",
      null, // Placement Details — current page
      "/attribution-lite/new?step=funding",
      "/attribution-lite/taxonomy/review",
    ];
          ...
          {viewMode === "taxonomy" ? (
            <>
              ...
              <div className="mt-8 flex items-center justify-between">
                <Link href="/attribution-lite/new?step=map-partners" className="rounded-lg border border-[#2d46f6] px-5 py-2 text-sm font-medium text-[#2d46f6] no-underline">Back to Partner Mapping</Link>
  ...
  return (
    <>
      ...
      <div className="mt-8 flex items-center justify-between">
        ...
        <Link href="/attribution-lite/taxonomy/review" className="rounded-lg bg-[#2d46f6] px-5 py-2 text-sm font-medium text-white no-underline hover:bg-[#2438d4]">Continue to Billing Information &gt;</Link>
      </div>
    </>
  );
}
  ...
  return (
    <header className="flex h-14 items-center justify-between border-b border-border bg-white px-12">
      <div className="flex items-center gap-8">
        <Link href="/attribution-lite" className="flex items-center gap-0.5 no-underline">
        ...
        <nav className="flex items-center gap-6">
          <Link href="/attribution-lite" className="text-sm font-semibold text-[#000033] no-underline">Dashboard</Link>
          <span className="text-sm text-[#6b7280]">Feasibility Calculator</span>
          <Link href="/attribution-lite/admin" className="text-sm text-[#6b7280] no-underline">Admin</Link>
        </nav>
      </div>
                    ...
                    <div className="flex flex-col">
                      {["Profile", "Admin Dashboard", "Permission Groups", "Impersonate"].map((item) =>
                        item === "Permission Groups" || item === "Admin Dashboard" ? (
                          <Link key={item} href={item === "Permission Groups" ? "/attribution-lite/permission-groups" : "/attribution-lite/admin"} className="w-full rounded px-2 py-1.5 text-left text-sm text-[#000033] no-underline transition-colors hover:bg-gray-50" onClick={() => setUserMenuOpen(false)}>{item}</Link>
                        ) : (
